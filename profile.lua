...
-- Sets are found as byte ranges in the source, so an edit splices back without touching
-- anything else. The sandbox supplies values; the text is what gets written back.
...
-- A shared include declares against its own name (blsets.sets = T{), matched by shape; the case
-- of the key is not fixed.
...
-- Exact byte ranges into the real file, so an edit splices those bytes and nothing else.
...
-- Found in StripComments output, which blanks comments at the same length, so a commented-out
-- set cannot match and every index maps back.
...
-- Quotes are walked past so a brace inside an item name cannot end a set early.
...
-- The range starts AFTER the sets. head, so the text is shaped like an entry in the literal;
-- only the writer knows, through entry.Form. An assignment inside a function body is skipped:
-- it only exists while the game runs.
...
-- The writer and the parser both go through here, so they cannot disagree about what counts as
-- a set.
M.ScanProfileKeys = function(text)
    ...
    if (varExpr ~= nil) then
        ...
        for _, e in ipairs(M.ScanStatementSets(text, varExpr, tableEnd + 1)) do
            -- A statement reassigning a name the literal declares is a runtime overwrite; the
            -- literal stays authoritative.
...
-- A set that carries a comment is spliced instead of regenerated.
...
-- The SAME LENGTH as the original, so every index in the stripped copy points at the same byte
-- of the file.
...
-- A profile that assigns to string.format would otherwise change it for the whole Lua state.
...
-- Without it any profile in the house style throws on its first line.
...
-- Answers any index and any call without throwing, so a profile pulling gear from a missing
-- framework still shows its sets.
local function MakeProxy()
    ...
    setmetatable(p, {
        ...
        -- Two DISTINCT values: profiles multi-assign from a framework call, and the same proxy
        -- twice lets profile.Sets = sets contain itself, which overflows any recursive walk.
        __call = function()
...
-- False for an entry carrying augments or any other descriptor: picking the same name there
-- drops those.
...
-- profile.Sets = gcmelee.AppendSets(sets) leaves the table a local on the far side of the
-- chunk. Blanking the local keyword to spaces makes it a global in the sandbox env with every
-- byte offset intact; nothing on disk changes.
...
M.RuntimeSets = function(result, env, localName)
    ...
    if (type(env) == 'table') and (localName ~= nil) then
        ...
        -- A proxy that has been written into holds real keys; an empty one stays rejected,
        -- since every read on it fabricates another proxy.
...
-- The reader and the save validator both go through here.
...
M.MakeSandboxEnv = function()
    -- Ashita builds LuaJIT with 5.2 compatibility, so profiles may call table.unpack and
    -- table.pack.
    local tableCompat = {};
    ...
    -- An unknown global answers with a proxy rather than nil, so profiles referencing framework
    -- globals still build their sets. Writes land in env first, so a file's own globals never
    -- see a proxy.
...
-- The raw source of every slot, so a slot filled by code is written back verbatim rather than
-- as what the sandbox evaluated.
...
-- A save rebuilds a set in screen order, so a comment is anchored to a slot name or the set's
-- own line and re-emitted with it. Returns { text, anchor, place } in file order: anchor is a
-- lowercase slot, SET_ANCHOR or nil; place is 'trailing' or 'own'.
M.SET_ANCHOR = '@set';
...
M.ExtractComments = function(slice)
    ...
    -- Walked by character position: splitting on a newline pattern desynced the stripped copy
    -- on every CRLF file.
    local starts = { 1 };
        ...
        if (s ~= nil) then
            ...
            if (body ~= '') then
                if (before:match('^%s*$') == nil) then
                    ...
                    -- A commented out slot goes back beside the slot it names. Otherwise it
...
local function BuildSetModel(name, value, scanEntry, text, depth, path)
    ...
    -- An expression evaluating to nil never creates a runtime key, so anything the text
    -- declares is carried through as code and written back verbatim.
...
-- The sandbox knows the values, only the scan knows where they live.
