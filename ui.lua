...
local function DrawSetViewColumn(setViewW)
    ...
    if (anyEntries == 0) and prof.IsEditable(S.model, nil) then
        ...
        if imgui.Button('Create an Idle Set', { CHOICE_WIDE_W, 0 }) then
            if (not NameInUse('Idle')) then
                CreateSet('Idle');
            end
        end
        imgui.SameLine();
        if imgui.Button('New Set', { CHOICE_WIDE_W, 0 }) then
            S.nameBuf[1] = '';
...
--[[
* Everything the addon can tell about the profile without running it: dead set
* references, handlers that read an action they never have, and sets nothing in this
* file mentions. Findings are ordered by whether they are already broken.
--]]
--[[
...
function DrawRuleFindings()
    ...
    if (S.analysis == nil) then
        --[[
        * Which framework helpers read the current action is learned from the
        * includes on disk, so it stays true if BasicLuas adds another. Read
        * once per analysis, which is cached until an edit invalidates it.
        --]]
        local helpers = {};
    ...
    * under here too, so the header draws even at zero.
    ...
    local faults = #S.analysis.typos + #(S.analysis.actionless or {})
        + #(S.analysis.badLevelCalls or {}) + #S.analysis.dead;
    local errors = #S.analysis.typos + #(S.analysis.actionless or {})
        + #(S.analysis.badLevelCalls or {});
    --[[
    ...
    * and is not broken. Red stays for the three that are.
    ...
    if (#S.analysis.typos > 0) then
        imgui.TextColored(theme.col.danger, 'Matches No Set');
        imgui.SameLine();
        theme.Hint('Your code asks for these sets,\nbut this file has none of them.\nNothing swaps. Usually a typo.');
        local editableProfile = prof.IsEditable(S.model, nil);
        imgui.Indent();
        for i, t in ipairs(S.analysis.typos) do
            imgui.Text(t.name);
            imgui.SameLine();
            imgui.TextDisabled(string.format('line %d, in %s', t.line, t.handler));
            local creatable = editableProfile and (t.kind ~= 'prefix')
                and (string.find(t.name, '.', 1, true) == nil)
                and writer.ValidateName(t.name) and (not NameInUse(t.name));
            if creatable then
                imgui.SameLine();
                imgui.PushID('mktypo' .. i);
                if imgui.SmallButton('Create This Set') then
                    CreateSet(t.name);
                    S.analysis = nil;
                    S.rulesCache = nil;
                            end
    ...
    --[[
    * Listed before every other finding because it is the only one that is
    * already broken rather than merely suspect: it throws once per frame from
    * the moment the profile loads. It cost a real user an evening, with
    * CheckCancels sitting in HandleDefault.
    --]]
    local actionless = S.analysis.actionless or {};
    if (#actionless > 0) then
        imgui.TextColored(theme.col.danger, 'Errors Every Frame');
        imgui.SameLine();
        theme.Hint('These read your current action\nfrom a handler that has no action.\nThe game errors every frame.');
        imgui.Indent();
        for _, a in ipairs(actionless) do
            imgui.Text(a.what);
            imgui.SameLine();
            imgui.TextDisabled(string.format('line %d, in %s', a.line, a.handler));
        end
        imgui.Unindent();
    ...
    if (#bad > 0) then
        imgui.TextColored(theme.col.danger, 'Priority Lists Never Run');
        imgui.SameLine();
        theme.Hint('There is no gData.EvaluateLevels,\nso _Priority sets are unused.\nThe tutorial got this wrong.');
        imgui.Indent();
        for _, b in ipairs(bad) do
            imgui.Text('gData should be gFunc');
            imgui.SameLine();
            imgui.TextDisabled(string.format('line %d', b.line));
        end
        imgui.Unindent();
    ...
    if (#S.analysis.dead > 0) then
        ...
        -- Naming the file turns an accusation into an explanation. 92% of these
        -- findings across the corpus are in a profile that hands its sets off.
        ...
        * rewrites the whole body and carries what was already wired into Otherwise.
        ...
        local wireAt = (body ~= nil) and (#body.equips == 0) and body.s or nil;

        imgui.Indent();
        for i, deadName in ipairs(S.analysis.dead) do
            imgui.TextDisabled(deadName);
            if (wireAt ~= nil) then
                imgui.SameLine();
                imgui.PushID('wire' .. i);
                if imgui.SmallButton('Use in HandleDefault') then
                    FinishSave(S.model.textAtLoad, { { op = 'insertline', at = wireAt,
                        line = '    gFunc.EquipSet(sets.' .. deadName .. ');' } });
                    S.analysis = nil;
                    S.rulesCache = nil;
                end
                imgui.PopID();
        ...
        if (body ~= nil) then
            if imgui.SmallButton('Set Up All Three') then
                ...
                S.wireSetup = {
                    idle = body.equips[1] or GuessSet({ 'Idle' }),
                    resting = GuessSet({ 'Resting' }),
                    engaged = GuessSet({ 'Tp_Default', 'Tp', 'Engaged', 'TP_Default' }),
                    span = body,
                };
            end
            if imgui.IsItemHovered() then
        ...
        --[[
        * Nothing is said when HandleDefault holds real logic, and that is the
        * point rather than an omission. It used to print a line here, which had
        * two problems: it sat at the same indent as the set names so it read as
        * another unused set, and EVERY finished profile has logic in that
        * handler, so it fired for everyone while only ever meaning anything to
        * somebody hunting for a button that was not there.
        --]]
        imgui.Separator();
        if (S.analysis == nil) then
            return;
...
local function DrawUnsavedModal()
    ...
    if imgui.BeginPopupModal('Unsaved Changes##lsv', nil, ImGuiWindowFlags_AlwaysAutoResize) then
        ...
        if imgui.Button('Save Them First', { CHOICE_W, 0 }) then
...
local function DrawFileChangedModal()
    ...
    if imgui.BeginPopupModal('File Changed##lsv', nil, ImGuiWindowFlags_AlwaysAutoResize) then
        ...
        if imgui.Button('Save Mine Anyway', { CHOICE_W, 0 }) then
            S.conflictNames = nil;
        ...
        imgui.SameLine();
        if imgui.Button('Cancel, Keep Editing', { CHOICE_WIDE_W, 0 }) then
            S.conflictNames = nil;
            S.pendingSave = nil;
            S.afterSaveOpen = nil;
            imgui.CloseCurrentPopup();
