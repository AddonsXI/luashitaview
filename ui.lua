...
--[[
* The handlers that fire on an action, so a bare EquipSet is the whole correct answer
* in them and no condition is needed.
*
* HandleDefault is deliberately absent even though it is the one people ask about: it
* runs every frame, so an unconditional line there means always and a second one
* silently overrides the first. It keeps the three state setup instead. OnLoad,
* OnUnload and HandleCommand are not about equipment at all.
--]]
...
*
* Asked for by an outside user on 19 Aug 2026, who had built sets, watched them do
* nothing and gone looking through the source for somewhere to put them. He described
* dragging a set onto the handler; a list on the handler itself is the same act with
* nothing to discover.
--]]
local function DrawWearHere(name)
    ...
    --[[
    * One control with the label inside it, rather than a caption and a box beside it.
    * The pair ran off the right of the column at any ordinary window width, and the
    * dropdown was the half that went, so the thing you had to reach was the thing you
    * could not see. It also shrinks with the column now instead of insisting on 180.
    --]]
    local wrote = false;
    ...
    for _, h in ipairs(S.analysis.handlers) do
        ...
        if imgui.CollapsingHeader(h.name .. '##rules') then
            ...
            if (lines ~= nil) then
                ...
                * rather than as more of our interface. Loose under a dropdown it
                * looked like the page had a fault on it.
                *
                * The horizontal scrollbar is the point of the box as much as the
                * color is: a line of profile code is wider than this column and
                * used to simply run out of sight.
                --]]
