...
--[[
* How to name a set in code we write. Dot form where the name is an identifier, bracket
* form otherwise.
*
* Not cosmetic: a set really can be called 'EP Set', and sets.EP Set does not compile, so
* every caller that built the reference by concatenation was one space away from writing
* a file that ValidateResult would refuse. Found 19 Aug 2026 on a real profile.
--]]
...
--[[
* How to write a set's own key when we create one. Same question as SetRef asks of a
* reference, so both go through one test.
*
* A set we create should look like the ones already in the file. Every set a person
* wrote is Name = {, and the bracket form is only actually needed when the name is not
* a plain identifier. The user's own WHM profile carried exactly one bracket key, the
* empty Stun set the addon made, against 31 plain ones they wrote themselves.
--]]
