...
-- A superset of what LuAshitacast writes: array slots, BaseSet, NoWrite, Priority, Level and
-- Quantity are kept, and augment strings are escaped.
...
M.SerializeSet = function(setModel, opts)
    ...
    -- Comments come back through their anchor, since the set is rebuilt in screen order. No
    -- anchor parks them at the end.
    local byAnchor = {};
    local trailingFor = {};
    local pending = 0;
    ...
    -- Everything unplaced, in file order: no anchor, or an anchor no loop above emits (BaseSet,
    -- NoWrite, Rings, Ears).
    if (pending > 0) then
        for _, c in ipairs(setModel.comments or {}) do
            if (not c.done) then Emit(c); end
...
M.ReplaceSet = function(text, name, body)
    ...
    if (entry.Form == 'statement') and (text:byte(entry.StartIndex) ~= string.byte('[')) then
        -- A dot-declared statement's range starts at the Name, so a bracket-key body would
        -- splice into sets.['Name']. A name that cannot be a dot key widens the splice to the
        -- head.
...
M.AppendSet = function(text, body)
    ...
    -- Only entries inside the literal are appended after; past the closing brace the new set
...
M.ValidateResult = function(text, expectedNames)
    ...
    -- Finding the sets through the file's own local does not prove the file returns anything
    -- LuAshitacast can load.
...
-- Both names are five characters, so every byte offset survives; matched in the stripped copy
-- so prose is never rewritten.
...
-- Suite only: LAC splits a set table on commas, so BasicLuas' stray semicolons are normalized
-- before comparing at identical indices.
-- Back to front so every index stays valid. Refuses if a span no longer holds the old name.
...
-- Anything unchosen is left out, so picking one set produces one line.
local Reserved = {
...
-- Bare where the name is a plain identifier, bracket form otherwise.
...
-- An empty span, s one past e, is an insert.
...
-- LuAshitacast picks the first _Priority entry whose level you meet and never looks in your
-- bags, so an unowned piece wins its slot and the slot stops swapping. Every addon has its own
-- Lua state, so the fix has to live in the file. This block ships into a stranger's profile,
-- which is why it is commented the way it is.
...
-- BasicLuas keeps a global CurrentLevel and rebuilds when it does not match the synced level,
-- so zeroing it forces the rebuild. A native profile calls EvaluateLevels itself, which mutates
-- in place.
local FILTER_TAILS = {
...
-- A hand written filter and ours would each snapshot the other's filtered lists as the
-- original, so gear bought later is wiped again. The signal is reading the bags and testing
-- names for _Priority.
...
-- By any route. OwnedFilterPresent means only our block, which is all Turn Off can take out.
...
-- By exact text, since the file may have been edited. Both pieces or nothing.
