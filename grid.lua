...
--]]
--[[
* available is an optional set of container ids this character has, used to grey out the
* bags they do not. Passing nil enables everything, which is what happens when the game
* cannot be asked: greying out every bag on a failed read would be worse than none.
*
...
local function DrawChainDetail(setModel, slotName, slotValue, editable)
    ...
    * the list was drawn, which is the bug that cost hours in August.
    ...
    for i, e in ipairs(slotValue.entries) do
        ...
        -- The position first, then the icon, so a row reads the same way round as
        -- one in the picker: a mark, then the piece, then its name.
        ...
        * out to the right of everything they belong to.
        --]]
        local avail = math.min(imgui.GetContentRegionAvail(), GRID_W);
        local nameW = math.max(90, avail - 92);
        ...
        if editable then
            imgui.SameLine();
            -- Drawn arrows rather than the characters ^ and v, which sit at
            -- different heights and read as two unrelated buttons.
            ...
            imgui.SameLine();
            if imgui.Button('X##rm' .. i, { btnH, btnH }) then
                removeAt = i;
            end
            if imgui.IsItemHovered() then
    ...
    if editable then
        if imgui.Button('Add Item', { BUTTON_W, 0 }) then
            OpenPickerToAppend(setModel, slotName);
        end
        imgui.SameLine();
    end
    if imgui.Button('Close Details', { BUTTON_W, 0 }) then
        S.detailSel = nil;
    end
end
...
local function DrawSetHeader(setModel, editable)
    ...
    * times in 1,316.
    ...
    if (setModel.pairedWith ~= nil) then
        imgui.SameLine();
        if imgui.SmallButton('Twin: ' .. setModel.pairedWith) then
            S.selected = setModel.pairedWith;
            S.detailSel = nil;
        end
        if imgui.IsItemHovered() then
            imgui.SetTooltip('Shares a purpose with its twin.\nClick to open it.');
        end
    end
    if (setModel.baseSet ~= nil) then
        imgui.SameLine();
        if imgui.SmallButton('Inherits: ' .. setModel.baseSet) then
        ...
        if imgui.IsItemHovered() then
            imgui.SetTooltip('Faded equipment comes from here.\nClick to open it.');
        end
    end
    --[[
    * Last on the row. The set name already ends in _Priority, so this only confirms
    * what the name said and has no business sitting between the name and everything
    * that can actually be done with it.
    --]]
    if setModel.isPriority then
        imgui.SameLine();
        theme.Badge('Priority List', 'accent', 'accentBg',
            'Used in game as ' .. setModel.logicalName .. '.\nSlots can hold item lists. The game wears the first item\nyour level allows, even if you do not own it.');
    end
end
