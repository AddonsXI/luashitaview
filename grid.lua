...
--[[
* A chip carrying a set name can be wider than the whole block on its own, and no
* amount of wrapping fixes that: it is one item, so there is nothing to wrap it onto.
* The name inside it is shortened until it fits, and the caller puts the full name in
* the tooltip, where none of them carried it before.
*
* limit and measure are arguments so the rule can be checked without ImGui. Nothing
* here reads the row it is drawing into, only a fixed width, which is what keeps this
* out of the measure-yourself-to-place-yourself trap the rest of this column learned.
--]]
local ELLIPSIS = '...';
...
*
* The order is kept as it was, the two reorder arrows together and then the remove,
* rather than putting the destructive one first. Leading the row already moves it well
* clear of the name, which is the thing you actually click.
--]]
...
local function DrawChainDetail(setModel, slotName, slotValue, editable)
    if setModel.isPriority then
        -- Carries the explanation the header badge used to, and is a better home for
        -- it: this line is on screen exactly when somebody is looking at a list.
    ...
    * the list was drawn, which is the bug that cost hours in August.
    ...
    for i, e in ipairs(slotValue.entries) do
        ...
        --[[
        * The reorder controls lead the row rather than trailing it.
        *
        * Pinned to the right they sat against the grid's edge while the name they
        * belong to ended wherever its text ran out, so a short name left a wide gap
        * with the buttons stranded across it. Leading, they are always the same
        * distance from the piece they act on, whatever it is called, and the name is
        * then free to take the rest of the row.
        --]]
        ...
        * e.level is a different number: the level the profile pins this entry to.
        ...
        --[[
        * The name is the button that opens this entry's own fields. Sized to leave
        * the reorder buttons their space, since a Selectable otherwise spans the
        * full width and swallows them.
        *
        * Capped at the grid's width so the row ends where the grid ends. The column
        * is wider than the grid, and left to fill it the buttons ended up stranded
        * out to the right of everything they belong to.
        --]]
        -- Whatever the row has left after the controls, the number and the icon, so
        -- it ends on the grid's edge like every other row in this column.
    ...
    --[[
    * Sized off the grid rather than left at two fixed 110s, which ended 100px short of
    * every other row in this column and was the one thing making the block look ragged.
    *
    * Measured from GRID_W and not from the content region: this panel is drawn inside
    * the block's indent, so the region runs wider than the grid and a -1 button would
    * overshoot the edge the grid, the slot buttons and the list rows all share.
    --]]
    if editable then
...
local function DrawSetHeader(setModel, editable)
    --[[
    * The chips after the name go on this row while they fit inside the grid's width,
    * and start a new row when they do not.
    *
    * It used to be an unconditional SameLine for every one of them, so a long set name
    * carrying a couple of chips ran off the right edge of the block that the grid, the
    * slot buttons and the list rows all share, and the last chip was simply clipped.
    *
    * Only the chip is measured against a fixed width. Nothing here reads the row's own
    * height or feeds a measurement back into a position, which is what made an earlier
    * version of this column drift a pixel every frame.
    --]]
    local rowW = 0;
