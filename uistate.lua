...
--[[
* The editing state and everything that changes it: the open profile, the undo stack,
* dirty tracking and the save pipeline. Draws nothing and never touches ImGui, so the
* whole module runs under the headless suite.
*
* ui, grid and sidebar all share the one S table exported here.
--]]
...
-- 0.57 left the gear grid 314px at the default window size, about 35 short of
-- the 350 it needs, so a fresh install opened with the last column of slots cut
-- off. This gives the grid room and the picker still gets the largest share.
local DEFAULT_SPLIT_B = 0.62;

-- Exported so the addon's default config uses these same numbers. Two copies of
-- a default is one copy too many: the config value is what actually wins on a
-- fresh install, so a drift here would silently undo any change made above.
...
local S = {
    ...
    rulesCache = nil,
    --[[
    * Bumped every time the window is opened, and every collapsible section closes
    * itself once per value. Nothing about what you expanded survives the window being
    * shut, on the user's call 19 Aug 2026: you open it to a clean panel rather than to
    * whatever you left open days ago.
    *
    * An epoch rather than a flag because a section only gets to close itself on a frame
    * it actually draws, and a section nested under a closed one may not draw for a long
    * time. A flag cleared at the end of the frame would miss every one of those.
    --]]
    openEpoch = 0,
    wasOpen = false,
    handlerEpoch = {},
...
M.OpenProfile = function(path)
    ...
    --[[
    * The set count is a FACT about the open file, not feedback about something
    * that just happened, so ui renders it beside the profile name rather than
    * writing it here. Opening a profile used to overwrite whatever the status
    * line was saying, which is how a save result vanished a frame after a save.
    --]]
    S.status = nil;
...
local function SelectedSet()
    ...
    -- A set inside a folder is addressed by the folder path the sidebar stored, walked
    -- segment by segment because folders nest. When the path no longer resolves, the
    -- plain lookup below still answers.
...
--[[
* Snapshots the whole set table onto the undo stack. Must be called BEFORE the mutation
* it protects: doing it from inside MarkDirty would be too late, since most sites
* change the model first and mark it afterwards.
--]]
PushUndo = function()
    if (S.model == nil) then
        return;
    ...
    --[[
    * A snapshot is a deep copy of every set, so the ceiling is about memory rather
    * than about how far back anyone wants to go. Measured at 127 KB per snapshot
    * on a 67 set profile and 49 KB on a small one, so a hundred of them costs
    * about 13 MB at worst and none of it comes back until the addon is reloaded.
    *
    * It used to be twenty, which also decided whether Save went dark: undo restored
    * the dirty flag along with the gear, so once the clean snapshot fell off the end
    * there was no way back to it. That is no longer true, since changed is asked
    * rather than remembered, and this is now only a limit on undo itself.
    --]]
...
--[[
* Undo and redo hand the current state to the opposite stack before restoring, so the
* two are mirrors and a step can be walked back and forward as often as you like.
--]]
...
--[[
* Whether a set differs from how it was when the file was opened.
*
* Asked rather than remembered. The dirty flag only ever went true, and undo put it
* back only because a snapshot happens to carry the flag along with the gear, which
* stops working the moment the twenty deep undo stack drops the clean one. Editing a
* piece and putting it back then left Save lit with nothing to save.
*
* dirty still means touched at least once, which is what decides whether a set needs
* its original text kept for the diff.
--]]
...
--[[
* Framework files this profile loads that do not exist on this install. Sending such a
* profile to the real LuAshitacast is not a failed load, it takes the whole addon down:
* the missing gFunc.LoadFile returns nil, the profile indexes it at file scope, and
* Ashita unloads LuAshitacast on the error. The sandbox never notices, because it
* proxies every load, which is why the reload button has to check the real disk.
--]]
--[[
* ⛔ THE PROFILE'S OWN FOLDER IS CHECKED FIRST, AND MISSING THAT REPORTED A REAL
* USER'S WORKING PROFILE AS BROKEN.
*
* LuAshitacast keeps profiles in either of two layouts and its LoadFile
* (func.lua:458) tries six paths, of which the character folder
* <root>\<Name>_<Id>\ comes BEFORE the root. A player using that layout has
* common\blinclude.lua inside their character folder, so a root-only check says
* it is missing when the game finds it fine.
*
* profileDir is the directory of the open profile, which for that layout IS the
* character folder, so checking it first mirrors LAC without having to know the
* player's name or id.
--]]
--[[
* Every framework file this profile loads, as text, so the Rules tab can learn
* which of their helpers read the current action rather than guessing.
*
* Same two bases as MissingFrameworkFiles and for the same reason: the profile's
* own folder first, because a character-folder layout keeps its includes there.
--]]
...
-- LuAshitacast only walks _Priority sets when the profile calls gFunc.EvaluateLevels,
-- or reaches it through BasicLuas' CheckLevelSync.
-- gData has no EvaluateLevels, so a profile carrying that spelling reaches nothing and a
-- plain substring test would wrongly report the lists as live.
...
--[[
* Where a renamed set is referred to elsewhere in the file, as spans to rewrite.
*
* The set's own declaration is excluded: for a statement-declared profile that head reads
* sets.Name and would match, and the writer already rewrites it when replacing the set,
* so leaving it in would rename it twice.
--]]
...
local function BuildEdits()
    ...
    --[[
    * References are rewritten BEFORE any set is replaced, because these are byte spans
    * measured against the file as loaded and a set replacement changes its length.
    * Replacements are safe afterwards: they find their set by name rather than by index.
    --]]
    for _, s in ipairs(S.model.sets) do
...
local function FinishSave(baseText, edits)
    ...
    S.reloadArmed = true;
    --[[
    * The status says the least that is still true. A backup is always taken and
    * the Reload button lights up on its own, so neither needs saying here. The
    * chat line keeps the count, where there is room for it.
    --]]
...
--[[
* The entry LuAshitacast would actually equip from a slot: the first one in file order
* whose level requirement is met. It deliberately does NOT consider what you own,
* because the game does not either, and that gap is the whole problem worth warning
* about.
--]]
