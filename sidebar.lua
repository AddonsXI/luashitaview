...
--[[
* The set list column: the fuzzy filter, the name-family grouping and each row with
* its marks and tooltip. ui draws the column and calls in here for its contents.
--]]
...
--[[
* Sorts the sidebar's visible entries into headed sections and loose leftovers. Section
* headings are alphabetical, but inside a section the file's own order is kept: an
* author writes Tp_Default before Tp_Acc, and sorting would bury the one they reach for
* most. The one exception is a family's base set, which leads its own section.
--]]
...
M.GroupForSidebar = function(visible)
    ...
    -- A set named exactly like a family IS that family's base set. With RA_Acc and
    -- RA_Multishot present, the set called RA belongs at the top of the RA section
    -- rather than stranded in Other, away from the family it named.
    for _, e in ipairs(visible) do
    ...
    --[[
    * A profile that has an Idle set almost always wants it first: 45% of the 347
    * readable corpus profiles open with one and 76% have one at all. Weapon is the
    * next most common opener at 16%, in the third of profiles that have one.
    *
    * Only if it exists. A name that is not in this profile pins nothing, so nobody
    * gets a heading for a set they do not have.
    --]]
...
local function DrawSetRow(s, parentPath)
    ...
    -- Green for the row you are on, the same green the picker uses for a piece
    -- already in the slot, so selection reads the same everywhere.
    if isSelected then
    ...
    --[[
    * The marks follow the name rather than the right edge of the column. Pinned to
    * the edge they end up a long way from the set they describe on a wide window.
    *
    * Each is the first character of the thing it stands for as it appears in the
    * file: P for a _Priority set, B for a BaseSet, { for a slot filled by code.
    --]]
    local nameW, textH = imgui.CalcTextSize(s.name);
    local x = minX + nameW + MARK_GAP;
    -- Centered against the row rather than pinned near its top, so a mark sits on
    -- the same line as the name instead of riding above it.
